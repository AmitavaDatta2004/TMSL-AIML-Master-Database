...
export async function GET() {
  try {
    ...
    // 3. Load Excel workbook from file buffer
    ...
    // 5. Construct 1-Row Matrix of size [1, 86]
    ...
    row[2] = student.roll_number || ""; // B.TECH UNIVERSITY ROLL NUMBER
    ...
    row[6] = details.photo_pdf_link || ""; // PHOTO PDF LINK
    row[7] = details.gender || "MALE"; // GENDER
    row[8] = details.dob || ""; // DOB (DD-MM-YYYY)
    row[9] = details.blood_group || ""; // BLOOD GROUP
    row[10] = details.contact_residence || ""; // CONTACT RESIDENCE
    row[11] = details.contact_operational_1 || ""; // OPERATIONAL CONTACT 1
    row[12] = details.contact_operational_2 || ""; // OPERATIONAL CONTACT 2
    row[13] = student.email || details.email_operational_gmail || ""; // EMAIL GMAIL
    row[14] = details.email_secondary || ""; // EMAIL SECONDARY
    row[15] = details.linkedin_link || ""; // LINKEDIN
    row[16] = details.github_link || ""; // GITHUB

    // Class X Details
    row[17] = details.class_x_exam_name || ""; // EXAM NAME
    row[18] = details.class_x_pass_year || ""; // PASS YEAR
    row[19] = details.class_x_board || ""; // BOARD
    row[20] = details.class_x_school || ""; // SCHOOL NAME
    row[21] = details.class_x_medium || "ENG"; // MEDIUM
    row[22] = details.class_x_std_marks_pct || ""; // STANDARD MARKS %
    row[23] = details.class_x_actual_pct || ""; // ACTUAL PERCENTAGE
    row[24] = details.class_x_math_pct || ""; // MATHS %
    row[25] = details.class_x_science_pct || ""; // SCIENCE GROUP %
    row[26] = details.class_x_comp_app_pct || "N.A."; // COMPUTER APPLICATION %

    // Class XII Details
    row[27] = details.class_xii_exam_name || ""; // EXAM NAME
    row[28] = details.class_xii_pass_year || ""; // PASS YEAR
    ...
    row[32] = details.class_xii_std_marks_pct || ""; // STANDARD MARKS %
    row[33] = details.class_xii_actual_pct || ""; // ACTUAL PERCENTAGE
    row[34] = details.class_xii_math_pct || ""; // MATHS %
    row[35] = details.class_xii_physics_pct || ""; // PHYSICS %
    row[36] = details.class_xii_chemistry_pct || ""; // CHEMISTRY %

    // Diploma Details
    row[37] = details.diploma_exam_name || "N.A.";
    row[38] = details.diploma_rank || "N.A.";
    row[39] = details.diploma_stream || "N.A.";
    row[40] = details.diploma_pass_year || "N.A.";
    row[41] = details.diploma_college || "N.A.";
    row[42] = details.diploma_university || "N.A.";
    row[43] = details.diploma_pct || "N.A.";

    // Entrance Exam
    row[44] = details.entrance_exam_name || "WBJEE"; // ENTRANCE EXAM NAME
    row[45] = details.entrance_exam_rank || ""; // ENTRANCE EXAM RANK
    ...
    row[49] = details.sem_1_cgpa || ""; // SEM 1
    row[50] = details.sem_2_cgpa || ""; // SEM 2
    row[51] = details.sem_3_cgpa || ""; // SEM 3
    row[52] = details.sem_4_cgpa || ""; // SEM 4
    row[53] = details.sem_5_cgpa || ""; // SEM 5
    row[54] = details.btech_avg_cgpa || ""; // AVERAGE CGPA
    row[55] = details.btech_backlog || "NO"; // BACKLOG
    row[56] = details.btech_backlog_count || "0"; // BACKLOG COUNT
    ...
    row[80] = details.study_gap_years || "0"; // GAP YEARS
    ...
    // 6. Write student matrix into worksheet starting at row 5 (cell A5) to overwrite hints
    XLSX.utils.sheet_add_aoa(worksheet, dataMatrix, { origin: 'A5' });

    // Auto-adjust column widths based on the content of the sheet
    const colWidths: any[] = [];
    const range = XLSX.utils.decode_range(worksheet['!ref'] || "A1:CH6");
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      colWidths.push({ wch: Math.min(max_width + 2, 50) }); // Cap at 50 chars to prevent excessively wide columns
    }
