...
// Mock list fallback when Neon DB connection is not configured in local development
const MOCK_LIST = [
  {
...
export async function GET() {
  try {
    ...
    // 5. Construct 2D Matrix of size [students.length, 86]
    const dataMatrix = students.map((student: any, idx: number) => {
      const details = student.details || {};
      
      const row = new Array(86).fill(""); // Create row with 86 empty elements (Columns A to CH)
